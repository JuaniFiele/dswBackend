...
async function obtenerFechasOcupadas(req, res) {
    try {
        ...
        const horariosIds = horariosMedico.map(horario => horario.id).filter((id) => id !== undefined);
        // Filtrar solo atenciones que no fueron canceladas (cancellationDate: null)
        const atenciones = await em.find(Attention, {
        ...
        const fechasOcupadas = Object.keys(contadorFechas).filter(fecha => contadorFechas[fecha] >= totalHorarios);
...
async function getAttentionsByDate(req, res) {
    try {
        ...
        const occupiedConsultationHourIds = attentions.map((attention) => attention.consultationHours?.id);
        res.status(200).json({
